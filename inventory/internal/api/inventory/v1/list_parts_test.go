package v1

import (
	"github.com/brianvoe/gofakeit/v7"
	"github.com/mllbll/space-manufacture/inventory/internal/converter"
	"github.com/mllbll/space-manufacture/inventory/internal/model"
	inventoryV1 "github.com/mllbll/space-manufacture/shared/pkg/proto/inventory/v1"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
	"google.golang.org/protobuf/types/known/timestamppb"
)

func (s *APISuite) TestListPartsSuccess() {
	var (
		uuid           = gofakeit.UUID()
		name           = gofakeit.Word()
		description    = gofakeit.Paragraph(1, 3, 3)
		price          = gofakeit.Float64()
		stock_quantity = gofakeit.Int64()
		category       = inventoryV1.Category_CATEGORY_ENGINE
		tags           = []string{gofakeit.Word(), gofakeit.Word()}
		createdAt      = gofakeit.Date()
		updatedAt      = gofakeit.Date()

		manufacturer_name    = gofakeit.BeerName()
		manufacturer_country = gofakeit.Country()
		manufacturer_website = gofakeit.URL()

		uuids = []string{uuid}
		names = []string{name}
		categories = []inventoryV1.Category{inventoryV1.Category(category)}
		manufacturer_countries = []string{manufacturer_country}


		dimensions_length = gofakeit.Float64()
		dimensions_width  = gofakeit.Float64()
		dimensions_height = gofakeit.Float64()
		dimensions_weight = gofakeit.Float64()

		manufacture = &inventoryV1.Manufacturer{
			Name:    manufacturer_name,
			Country: manufacturer_country,
			Website: manufacturer_website,
		}

		demensions = &inventoryV1.Dimensions{
			Length: dimensions_length,
			Width:  dimensions_width,
			Height: dimensions_height,
			Weight: dimensions_weight,
		}

		metadata = map[string]*inventoryV1.Value{
    	gofakeit.Word(): {
        Types: &inventoryV1.Value_StringValue{
            StringValue: gofakeit.Word(),
        },
    },
    gofakeit.Word(): {
        Types: &inventoryV1.Value_BoolValue{
            BoolValue: gofakeit.Bool(),
        },
    },
    gofakeit.Word(): {
        Types: &inventoryV1.Value_DoubleValue{
            DoubleValue: gofakeit.Float64(),
        },
    },
    gofakeit.Word(): {
        Types: &inventoryV1.Value_Int64Value{
            Int64Value: gofakeit.Int64(),
        },
    },
	}

		part = &inventoryV1.Part{
			Uuid:           uuid,
			Name:           name,
			Description:    description,
			Price:          price,
			StockQuantity: stock_quantity,
			Category:       category,
			Dimensions:     demensions,
			Manufacturer:   manufacture,
			Tags:           tags,
			Metadata:       metadata,
			CreatedAt:      timestamppb.New(createdAt),
			UpdatedAt:      timestamppb.New(updatedAt),
		}

		partsFilter = &inventoryV1.PartsFilter{
			Uuids: uuids,
			Names: names,
			Categories: categories,
			ManufacturerContries: manufacturer_countries,
			Tags: tags,
		}

		listPartsResponse = &inventoryV1.ListPartsResponse{Parts: []*inventoryV1.Part{part}}

		listPartsRequest = &inventoryV1.ListPartsRequest{
			Filter: partsFilter,
		}

		expectedListPartsRequest = converter.ListPartsRequestToModel(listPartsRequest)

		expectedListPartsResponse = converter.ListPartsResponseToModel(listPartsResponse)
	)

	s.inventoryService.On("ListParts", s.ctx, expectedListPartsRequest).Return(expectedListPartsResponse, nil)

	res, err := s.api.ListParts(s.ctx, listPartsRequest)

	s.Require().NoError(err)
	s.Require().NotNil(res)
	s.Require().Equal(listPartsResponse, res)
}

func (s *APISuite) TestListPartsError() {
	var (
		serviceErr = gofakeit.Error()

		uuid           = gofakeit.UUID()
		name           = gofakeit.Word()
		category       = inventoryV1.Category_CATEGORY_ENGINE
		tags           = []string{gofakeit.Word(), gofakeit.Word()}
		manufacturer_country = gofakeit.Country()


		uuids = []string{uuid}
		names = []string{name}
		categories = []inventoryV1.Category{inventoryV1.Category(category)}
		manufacturer_countries = []string{manufacturer_country}

		partsFilter = &inventoryV1.PartsFilter{
			Uuids: uuids,
			Names: names,
			Categories: categories,
			ManufacturerContries: manufacturer_countries,
			Tags: tags,
		}

		listPartsRequest = &inventoryV1.ListPartsRequest{
			Filter: partsFilter,
		}

		expectedListPartsRequest = converter.ListPartsRequestToModel(listPartsRequest)
	)

	s.inventoryService.On("ListParts", s.ctx, expectedListPartsRequest).Return(model.ListPartsResponse{}, serviceErr)

	res, err := s.api.ListParts(s.ctx, listPartsRequest)

	s.Require().Error(err)
	s.Require().ErrorIs(err, serviceErr)
	s.Require().Nil(res)
}

func (s *APISuite) TestListPartsNotFoundError() {
	var (

		uuid           = gofakeit.UUID()
		name           = gofakeit.Word()
		category       = inventoryV1.Category_CATEGORY_ENGINE
		tags           = []string{gofakeit.Word(), gofakeit.Word()}
		manufacturer_country = gofakeit.Country()


		uuids = []string{uuid}
		names = []string{name}
		categories = []inventoryV1.Category{inventoryV1.Category(category)}
		manufacturer_countries = []string{manufacturer_country}

		partsFilter = &inventoryV1.PartsFilter{
			Uuids: uuids,
			Names: names,
			Categories: categories,
			ManufacturerContries: manufacturer_countries,
			Tags: tags,
		}

		listPartsRequest = &inventoryV1.ListPartsRequest{
			Filter: partsFilter,
		}

		expectedListPartsRequest = converter.ListPartsRequestToModel(listPartsRequest)
	)

	s.inventoryService.On("ListParts", s.ctx, expectedListPartsRequest).Return(model.ListPartsResponse{}, model.ErrPartNotFound)

	res, err := s.api.ListParts(s.ctx, listPartsRequest)

	s.Require().Error(err)
	s.Require().Nil(res)

	st, ok := status.FromError(err)
	s.Require().True(ok)
	s.Require().Equal(codes.NotFound, st.Code())
	s.Require().Contains(st.Message(), "part with your filter not found")
}
